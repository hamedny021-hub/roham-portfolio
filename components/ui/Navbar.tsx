      ...
      >
        ...
        <ul className="hidden md:flex items-center gap-9">
          {links.map(l => (
            <li key={l.href}>
              <a href={l.href}
                className="font-accent text-[10px] tracking-[0.22em] uppercase text-ivory/50 hover:text-gold/90 transition-colors duration-300 hover-gold">
                {l.label}
              </a>
            </li>
          ))}
          ...
          >
            {links.map((l, i) => (
              <motion.a key={l.href} href={l.href}
                ...
                className="font-brand font-bold text-4xl text-ivory hover:text-gold transition-colors duration-300">
                {l.label}
              </motion.a>
            ))}
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
              className="font-sans text-[10px] tracking-[0.3em] uppercase text-ivory/20 mt-8">
